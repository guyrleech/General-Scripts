...
04/08/22  @guyrleech  Added facility to copy file if cannot be accessed because busy
#>
...
try
{
    $fileStream = New-Object -TypeName System.IO.FileStream( $path , [System.IO.FileMode]::Open , [System.IO.FileAccess]::Read )
}
catch
{
    if( $makeCopy -and $_.Exception -and $_.Exception.GetBaseException() -match 'The process cannot access the file .* because it is being used by another process' )
    {
        if( $fileCopy = New-TemporaryFile )
        {
            Copy-Item -Path $path -Destination $filecopy.FullName -Force
            if( $? )
            {
                $fileStream = New-Object -TypeName System.IO.FileStream( $fileCopy , [System.IO.FileMode]::Open , [System.IO.FileAccess]::Read )
            }
            else
            {
                Remove-Item -Path $fileCopy -Force -ErrorAction SilentlyContinue
                Throw "File is busy but unable to take copy to $($fileCopy.FullName)"
            }
        }
        else
        {
            Throw "File is busy but unable to take a copy because couldn't get a temp file"
        }
    }
    else
    {
...
finally
{
    if( $fileCopy )
    {
        Remove-Item -Path $fileCopy -Force
    }
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQULvGGNnE3vmalBtFaMyc8wIyA
# u8iggh2UMIIFMDCCBBigAwIBAgIQBAkYG1/Vu2Z1U0O1b5VQCDANBgkqhkiG9w0B
...
# BgkqhkiG9w0BCQQxFgQUbpu7reKlJxl7tOipc7mrhfPAr34wDQYJKoZIhvcNAQEB
# BQAEggEALzYfc0J0Hi4eqUaU/XTKsOeOOipcTRFnAmh1yGXtDyz+RxtpynTHAvL3
# IhpIQDGHbhykmu8Vz5q+xiSJDzoUWjI/SuHigaQkYSnazkhqYgED9hYKRM+mbtuo
# N7TK1CSfuRDb/U9jqo/LmrL96t4psTvICcnc6mw3W+RX0iMUoFiDaTUMOvE+GLv6
# w3uctFh7EWr4+F3v3p8dsC0r+1QkDnPAtSibpOq683dheI2OIFOuIjVF7UTurY53
# +wtF2ZDQxgl4gH+kTi4/6TdNKCtLwFy1SUooskNT9wI4abL5KNrmceMn1qx46b5A
# EFRIev/ajJ1Bo50iZHzoMErVPXVI9KGCAyAwggMcBgkqhkiG9w0BCQYxggMNMIID
...
# BgkqhkiG9w0BCQMxCwYJKoZIhvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMjA4MDUw
# OTU2MjJaMC8GCSqGSIb3DQEJBDEiBCAysT2p8aMng0u3Ydo1y98vom3+Qa0EaLyo
# SJT1JGZUIjANBgkqhkiG9w0BAQEFAASCAgAYsQ1pB5tIUuI5fzScfV9OCYirLYjn
# he7XE+4iTZu0XcVWz0du5MsVnfaj4vzohfiQ3u4CBE9IxvjP3Q4IIAz3BCyUL0C5
# qrxVMTdiRkfFJsHOtcJLwRTMsoU8qVZo1o+JhQrkyXfFvj8fH8LqHz9tbclDnJnF
# AU1NFeSZd1oWQAXFqDTCsGUOzikU448oIRoAgFBTCBi8Ls6Db0naaU/hdpie9uyE
# mooOZjILfmBDttZH71K9p9OUIv7gRGqRlrEy8tRdpDsY3klrQ8XiF6UxL21MyPlR
# utXIcyDXm7vpMa1fNieoBKkmlWc2HeLcrBsudqpHmvQWAQco2VP9oyHdnLmc7vJa
# qR1CvOAzvacihriYk02bojYD/qfHjh2YI+/C27wU33HsbYPRNw24hxYKuw0VUsBZ
# S35f6mIqdKubOLs9hxfQgIJPEi8cjDskSF8OhXDdn+hFtj6pc1OzXevHM9JVL9X1
# +VJmdNzeOnDWYhPGsCANiU56cGGukMw+vBaZgPzpjlF5pWVySq+Jyow6U7wtveqP
# 8MFtFJGFK8KADnV/4/86kvpJ3pQLP60wQ/3KhFDxi/cQuwPcn4MBe/s24huB5rSk
# xac339oN4bzp8STP05N+dhgvX1UTkegmq5jrCg8ch7mZ/nk1+1EuK7EQlTDguua5
# Jiick8TR5aO9ww==
